...
import { Search, ShoppingCart, User, Menu, X, Phone, LogOut } from 'lucide-react';
...
export default function Header() {
  ...
  // 用 token 验证登录状态，token 不存在就清除 user
  useEffect(() => {
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <div className="hidden sm:flex flex-col">
                <span className="font-display text-2xl text-gray-800">QIYU</span>
            ...
            <form onSubmit={handleSearch} className="flex-1 max-w-xl mx-4 hidden md:block">
              <div className="relative">
                ...
                <button
                  type="submit"
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-full flex items-center justify-center hover:from-blue-700 hover:to-cyan-600 transition-all shadow-lg shadow-blue-500/30"
                >
              ...
              {user ? (
                <div className="flex items-center gap-2">
                  {user.role === 'admin' && (
                    <Link
                      to="/admin"
                      className="text-xs bg-brand-blue text-white px-3 py-1.5 rounded-full hover:bg-brand-dark transition-colors"
                    >
                      Admin
                    </Link>
                  )}
                  <Link
                    to="/account"
                    className="flex items-center gap-2 bg-gray-100 hover:bg-blue-50 text-gray-700 hover:text-brand-blue px-5 py-2.5 rounded-full font-body font-semibold transition-all duration-300"
                  >
                    <User size={18} />
            ...
            <div className="px-4 py-4 space-y-2">
              {user ? (
                <div className="flex gap-3 pb-4 border-b border-gray-100">
                  <Link
                    to="/account"
                    className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-lg font-body font-semibold text-center flex items-center justify-center gap-2"
                  ...
                  <button
                    onClick={handleLogout}
                    className="px-4 bg-red-50 text-red-500 rounded-lg font-body"
                  >
                    <LogOut size={18} />
                  </button>
                </div>
