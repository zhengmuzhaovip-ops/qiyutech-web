      ...
      <div className="mb-6 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-end sm:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-neutral-400">
            查看最新批发订单，更新订单和付款状态，并在同一个后台界面直接下载正式发票 PDF。
          </p>
        </div>
      </div>
                ...
                >
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-base font-medium text-white sm:text-lg">{order.orderNumber}</p>
                    ...
                    <div className="flex items-start gap-4 text-right">
                      <div>
                        ...
                        <p className="mt-1 text-xs uppercase tracking-[0.22em] text-neutral-500">
                          {getStatusLabel(order.status)} / {getPaymentStatusLabel(order.paymentStatus)}
                        </p>
                      </div>
                        ...
                        <div className="space-y-3">
                          {order.items.map((item) => (
                            <div
                              ...
                              className="grid gap-3 rounded-[1rem] border border-white/10 bg-white/[0.02] px-4 py-3 sm:flex sm:items-center sm:justify-between"
                            >
                              ...
                              <p className="text-sm text-neutral-300">${item.lineTotal.toFixed(2)}</p>
                            </div>
                          ))}
                        </div>

                        <div className="space-y-3">
                          <div className="rounded-[1rem] border border-white/10 bg-white/[0.02] px-4 py-4">
                            <p className="text-xs uppercase tracking-[0.18em] text-neutral-500">
                              用户信息
                            </p>
                            <p className="mt-3 text-sm text-white">{order.customer}</p>
                            <p className="mt-1 break-all text-sm text-neutral-400">{order.email || '暂无邮箱'}</p>
                            <p className="mt-1 text-sm text-neutral-400">{order.phone || '暂无电话'}</p>
                            <p className="mt-3 text-sm text-neutral-400">{order.addressLine}</p>
                          </div>
                          ...
                          <div className="rounded-[1rem] border border-white/10 bg-white/[0.02] px-4 py-4">
                            ...
                            <div className="mt-3 space-y-2 text-sm text-neutral-300">
                              <AdminRow label="商品小计" value={`$${order.subtotal.toFixed(2)}`} />
                              <AdminRow label="运费" value={`$${order.shipping.toFixed(2)}`} />
                              <AdminRow label="税费" value={`$${order.tax.toFixed(2)}`} />
                              <AdminRow label="订单总额" value={`$${order.total.toFixed(2)}`} />
                              <AdminRow label="付款方式" value={formatPaymentMethod(order.paymentMethod)} />
                            </div>
                          </div>
                          ...
                          <div className="rounded-[1rem] border border-white/10 bg-white/[0.02] px-4 py-4">
                            ...
                            <div className="mt-4 grid gap-3 sm:grid-cols-2">
                              <Button
                                ...
                                className="w-full"
                              >
                                {savingOrderId === order.id ? '保存中...' : '保存订单更新'}
                              </Button>
                              <Button
                                type="button"
                                variant="secondary"
